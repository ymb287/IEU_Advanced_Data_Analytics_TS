...
The graph shows the predicted GDP values, considering factors like trends and seasons. The positive outlook suggests Italy's GDP is on the rise after the 2007 crisis. Comparing it with Italy's Consumer Price Index (CPI), we find that the forecasting model used here works well for GDP trends. On the other hand, ARIMA is effective for predicting CPI, especially during the 2007 crisis. In conclusion the tool helps us see future economic trends, and for Italy's GDP, things are looking up. The forecasting methods used give us insights into economic patterns and potential future directions.
...
Our visual compares different models for forecasting Italy's CPI. While ARIMA underestimated actual data and both models performed well on training data but struggled with testing, the Winter-Holt model stood out in predicting CPI trends accurately. Despite the complexity of CPI influenced by fiscal and political factors, our models consistently predict a rise in CPI, aligning with increased GDP predictions. This insight is crucial for policymakers to prevent inflation and manage CPI effectively.
...
Our visual examines residuals (the differences between predicted and actual values) for the best ARIMA model forecasting Italy's GDP. Both plots, ACF and PACF, show no significant patterns in the residuals, indicating a good fit for the model. Because there are no issues, we can trust this model for predicting Italy's GDP without the need for further adjustments.
...
We tried different approaches to forecast Italy's Consumer Price Index (CPI). First, we manually selected an ARIMA(7, 1, 0) model based on significant lags. Then, we used auto ARIMA models, considering AIC and BIC for selection. The evaluation showed that the ARIMA(7, 1, 0) model had the lowest AIC, but its residuals didn't behave like white noise, indicating room for improvement. We also tested an additional ARIMA(6, 1, 0) model. In summary, our process involved trying various models, evaluating based on information criteria, and acknowledging the need for potential adjustments to enhance CPI forecasting accuracy.
...
The ACF and PACF visuals for Italy's CPI residuals indicate violations, notably with a significant spike in both plots, suggesting unaccounted patterns. These patterns may necessitate the consideration of GARCH models to address volatility and improve forecasting accuracy. Addressing these issues is crucial for enhancing the reliability of CPI forecasts, which, if unaddressed, could impact Italy's economic planning and decision-making.
...
We explored various seasonal ARIMA models to improve forecasting for Italy's CPI. Notably, models with different seasonal periods (4 and 6, 4 and 12, 2 and 6, 2 and 12, 1 and 6) were tested. Evaluation results showed that while the model with a seasonal period of 4 and 6 had the lowest AIC, its MSPE was high, suggesting less accurate predictions. This underscores the importance of considering both AIC and MSPE. Additionally, the visual observation that a seasonal pattern occurs every 6 periods aligns with our exploration of different seasonal periods. This analysis guides us in selecting the most effective model for precise CPI forecasting, with potential improvements compared to the initial model or the Winter-Holt method.
...
We see no violations in the PACF of CPI residuals in the USA and one spike in the ACF of CPI residuals in usa. Therefore we have good models and do not need further modeling. 
...
Because the squared returns, when plotted in ACF and PACF show some violations, it tells us that there is still information to be extracted from the data that our previous models did not capture. Therefore GARCH will be used to try and understand the remaining violations of the PACF and ACF.
...
Here we take into consideration several GARCH models to evaluate them by the lowest AIC, which leads us to the most accurate model.
...
This GARCH model was used due to observed volatility in the data, as observed in the high peak periods, with the housing crisis in 2007, and economic uncertainties in 2012. The model is very accurate and helps us visualize this element in the data. 

# Conclusion

In conclusion, our project reveals a close interrelation among the analyzed variables, particularly evident in the comparison between the US and Italy. Notably, the US GDP exhibited robust and non-seasonal growth, outpacing Italy. The US Consumer Price Index (CPI) also displayed a faster rate of increase. The distinct impacts of the housing crises on stocks and GDP further underscore the complexity of these interconnected economic factors.
