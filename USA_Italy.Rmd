...
knitr::write_bib(c("references.bib"))
```
...
In the flowing part a literate review about time series analysis in combination with the GDP, CPI and stocks, as well as the countries USA and Italy will be conducted.
It is already known in the economic community, that the economic growth and health of a country can not only be measured by the GDP, and an ARIMA analysis of this alone is not enough. This is also why sometimes countries can be growing in GDP and still have bad economic health. @cobb1995if] This is also the reason why we are using multiple variabels.
As a first step looking at the connection between GDP and inflation, literature revealed, that a too high inflation has a negative impact on the economy. The paper notes that the point at which a higher CPI affects GDP the most is at a CPI of 7%. Before that it does not seem to have a negative impact. [@ayyoub2011inflation] 
This can help policy makers to understand limits of CPI that should not be surpassed and where our analysis can help to predict them. The strong connection of GDP and CPI as well as their indication of economic health is also supported in [@ghazo2021arima], since they used the same approach for predicting economic growth. The paper discovered that increasing CPI and increasing GDP mean cost-push inflation, whereas decreasing GDP and rising cpi mean stagflation. [@ghazo2021arima]
Shifting focus to the stock market's relationship with economic growth, literature shows that the stock market is as a reliable indicator. This relationship is explored in many studies one of which is referenced by [@stockgdp].
Regarding modeling, ARIMA is still considered a very good approach and although it is outperformed by deep learning, it is still better with regards of computational time [@siami2018forecasting]. Also leaving out the covid data is supported by literature as well, because it is important to evaluate extraordinary events separately [@ijerph17082800]
...
library(pacman)
p_load(
  readxl, tseries, tidyverse, dplyr, tidyr, forecast, fpp2, stats, quantmod, zoo, lubridate, 
  ggplot2, cowplot, rugarch)
...
As observed, the years that we will be using for this analysis are from 2000 to 2020, giving us a comprehensive analysis of each country's variables throughout the 21st century. The data were collected monthly with the exception of GDP, which was collected quarterly.
...
## 1.3 Visual inspection of the data

In the next part we will analys the data visually. We will plot the data and zoom in on a 4 year period. This will give us a better understanding of the data and how it is distributed, as well as potential seasonality and trends.
...
We can make some observations by looking at the graph of Italy's GDP from 2000 - 2020. The GDP shows a clear upward trend, with a sharper increase from 2000 to 2008, where it stabilizes, and again an increase from 2015-2020. The interruption in the trend corresponds to the 2008 financial crisis, which shook the economy and its growth. We can also note a seasonality throughout the time series. Italy's economy after 2001 began to slow down, even though the GDP is seen to be growing, the rate of growth had decreased. Factors such as "aging population, inflation, stagnation, and the impact of globalization on trade and demand" heavily affected the country's development and economic growth. [@ardeni2023]

Italy's development is slow mainly when compared to other countries European countries with similar capabilities and factors. We can observe another factor in the zoomed version of Italy's GDP. The plot shows a clear trend or pattern of seasonality, wherein in the first part of the year we observe a slight increase in the GDP, followed by a sharper increase in the second part of the year, and this is observed in all of the years included. This could be attributed to factors such as traditional vacation patterns, tourism peaks, or specific industry behaviors.
...
To get a deeper understanding of suitable models we will evaluate the data. There the ACF and PACF, as well as stationarity is analyzed.
...
After using the recommended number of differences (1) on Italy's CPI the Augmented Dickey-Fuller (ADF) test suggested  stationarity, as well as no further differencing. But it can be seen that the value is still bigger then the other once before and taking the graphs into account as well a seasonal component can be expected.
...
For Italy's stock data the ADF has a p-value of 0.41 suggesting non-stationarity. Also it is recommended to difference once. The ACF shows a slow decline and since the PACF has a steep drop at lag 2, the model will probably be an AR(2). This will be tested in the modeling part.
...
plot_volatility_clusters(transformed_italy$stocks, 'Italy', 'Stocks', 175)
```
...
```
The volatility clustering of the US stock data highlights larger volatility during uncertain times such as the 2002 dot com bubble and 2007 housing crisis. Here the plot is zoomed cut of at a y value of 50 because otherwise the extream high volatility in the housing crisis would distort the plot. As a result of the volatility, GARCH is used as the preferred model to capture the most information.
...
The graph shows the predicted GDP values, considering factors like trends and seasons. The positive outlook suggests Italy's GDP is on the rise after the 2007 crisis. Comparing it with Italy's Consumer Price Index (CPI), we find that the forecasting model used here works well for GDP trends. In conclusion the tool helps us see future economic trends, and for Italy's GDP, things are looking up. The forecasting methods used give us insights into economic patterns and potential future directions, which is fitting with the test data. It suggests a rising gdp and incorporates also the seasonal pattern, which allows decision makers to formulate adequate policies for a continuing healthy growth.
...
Our visual compares different models for forecasting Italy's CPI. The Winter-Holt model stood out in predicting CPI trends accurately, but slightly overestimating it This could also mean that during the time of the test data decision makers already released decisions to slow down the inflation, which can be supported by the findings that during the time of 2015 inflation had a historical low, also the lower growth might have contributed to that. [@annualreport2015]

 This insight is crucial for policymakers to prevent inflation and manage CPI effectively.

## 2.2 ARIMA

In the next step for vaiable a ARIMA respectively GARCH is developed.
...
Our visual examines residuals (the differences between predicted and actual values) for the best ARIMA model forecasting Italy's GDP. Both plots, ACF and PACF, show no significant patterns in the residuals, indicating a good fit for the model. Because there are no violations this model captures the patterns in the data well and is suited for predicting Italy's GDP without the need for further adjustments.
...
This model can be used to predict the GDP of Italy and therefore guide policy makers to make decisions based on the forecaster values. They can be used to help a strong growing economy to keep growing or to help a weak economy to recover.
...
We tried different approaches to forecast Italy's Consumer Price Index (CPI). First, we manually selected an ARIMA(7, 1, 0) model based on significant lags. Then, we used auto ARIMA models, considering AIC and BIC for selection. The evaluation showed that the ARIMA(7, 1, 0) model had the highest AIC, and its residuals didn't behave like white noise, indicating overfitting. The best models were seasnal models with an SAR. But those as well did not have White Noise residuals.
...
The ACF and PACF visuals for Italy's CPI residuals indicate violations, notably with a significant spike in both plots, suggesting unaccounted patterns. These patterns may necessitate the consideration of larger models to improve forecasting accuracy. There it is important to prevent overfitting and one method is to drop not significant parameters to still keep the model as simple as possible, while using pattern in higher order lags. Only with accurate models decision makers can relay on the forecasted values.
...
We explored various seasonal ARIMA models to improve forecasting for Italy's CPI. Notably, models with different seasonal periods (6 and 12) with different orders of seasonal components (1,2, and 4) were tested. Evaluation results showed that while the model with a seasonal period of 6 and 4 parameters had the lowest AIC, its MSPE in the test data was high, suggesting less accurate predictions. This underscores the importance of considering both AIC and MSPE. Additionally, the visual observation that a seasonal pattern occurs every 6 periods aligns with our exploration of different seasonal periods. This analysis guides us in selecting the most effective model for precise CPI forecasting, with potential improvements compared to the initial model or the Winter-Holt method.

```{r plot residual italy cpi2, echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}

plot_residuals_arima(seasonal_4_6_i_c, "Italy", "CPI")

```

We see no violations in the PACF of CPI residuals in the USA and one spike in the ACF of CPI residuals in Italy. Therefore we have good models and do not need further modeling. 
...
plot_forecast_arima_3 <- function(train_data, test_data, ts_data, country, variable, wh, arima1, arima2) {
  ...
  ggplot(data.frame(time = time(ts_data), value = ts_data, pred_arima = arima1)) + 
    ...
    geom_line(aes(x = time, y = arima1, group = 2, color = "AutoArima"), size=0.75) +
    geom_line(aes(x = time, y = arima2, group = 3, color = "LowesAIC"), size=0.75) +
    ...
    theme(plot.title = element_text(size = 12, face = "bold", hjust = 0.5),
          plot.subtitle = element_text(size = 10, hjust = 0.5),
          plot.caption = element_text(size = 6)) +
    scale_color_manual(values = c("Actual" = "black", "WH" = "darkgreen", "AutoArima" = "orange", "LowesAIC" = "red"))
}
...
Here several ARIMA models were used alongside winter-holts to capture the trend and seasonality within the CPI data. Although all these models properly model the training data, the test data could not be captured accurately by our modeling. Overall Winter-holt had the best prediction for the direction of the trend. Also the differenc between the auto.arima model as well as the one with the lowes AIC wich has 6 periods for the seasonal componant perform very similar. This shows how important it is to use different methods. Here further combination might lead to the best result. When decision makers relay on only the Arima they might underestimate the inflaiton whereas the Winter-Holt method might overestimate it.
...
Because the squared returns, when plotted in ACF and PACF show some violations, it tells us that there is still information to be  extracted from the data and give a hint on the order of ARCH and GARCH.
...
Comparing the different models the lowes AIC was achived for the GARCH(1,1) model. This model was then used to predict the volatility of the stocks in italy.

```{r modelling garch italy stocks, echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}

vole <- ts(g1@fit$sigma^2,end=c(2015,12),frequency = 12)
plot(vole,xlab="",ylab="",main="Italy stocks Volatility (GARCH[1,1])")


```

GARCH model was used due to observed volatility in the data, as observed in 2000-2002 the model properly models the dot com bubble, and then in 2007 the housing  crisis. The model is also consistent with the crisis that  where modeled with the GDP, showing that the stock data of italy is closely related to the country's GDP data. This can help decision makers to use information from different data sources becuase it shows that the stock market can already early show signs of volatility and this often indecates a crisis which then the decision makers can use to prevent the crisis from happening.
...
Our model uses our best forecast, lowest aic model with ARIMA, and underestimates growth. This is clearly visible in the test data. Here models with higher growth components maybe even exponential growth components could be used to better model the data. It is important to know the limitations of the models and potential problems because decision makers need to know if the predictions are reliable. To make good use. But still with this model the USA can in the short term estimate the growth of the GDP properly.
...
These new models yield lower AIC results and different values for the other categories and give us better and more accurate overall results. Also here higher order models are creaed to caper the violations at later lags but the not significatn parameters are dropped.
...
Because the squared returns, when plotted in ACF and PACF show some violations, it tells us that there is information to be extracted from the data. For this ARCH and GARCH will be used. It also gives a first idea on what order the models might need to be
...
Here we take into consideration several ARCH and GARCH models to evaluate them by the lowest AIC, which leads us to the most accurate model.
...
We can see a very accurate model which captures the housing crisis as well as economic uncertainties in 2012 well. Also comparing his with the GDP in the USA it showed that this is again correlated and had a negative effect on the GDP. This correlation can again be used by decision makers to keep the economy healthy.

# Conclusion

In conclusion, our project reveals a close interrelation among the analyzed variables, particularly evident in the comparison between the US and Italy. Notably, the US GDP exhibited robust and non-seasonal growth, outpacing Italy. The US Consumer Price Index (CPI) also displayed a faster rate of increase. Both show that the economy in the USA is stronger, and the forecast might help to improve both economies further by simultaneously keeping the inflation rate in check. This balance is very important to maintain a healthy economy. Also, the distinct impacts of the housing crisis on stocks and GDP further underscore the complexity of these interconnected economic factors.

Looking ahead, both economies show an upward trend, but our models highlight a concerning rise in inflation. This emphasizes the importance of focusing on inflation in the fiscal policy agendas of both governments to ensure sustainable economic growth and stability.
