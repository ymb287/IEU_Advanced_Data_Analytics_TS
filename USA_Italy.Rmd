...
In our financial analysis of Italy's GDP, the Autocorrelation Function (ACF) displayed over 15 significant lags, as well as the before seen seasonal pattern. Due to the slow decline we might conclude that it is an AR model. This is further supported due to the steep decline in the Partial Autocorrelation Function (PACF) after the fifth lag. Also the Model will probably include a seasonal component. Since is is quarterly and could already be seen in the plotted data. This might even reduce the PACF lags to 1. The Augmented Dickey-Fuller (ADF) test has a p-value of 0.26, which is more than 0.05, indicating that the data is non-stationary. The number of differences required for stationarity is 1.
...
After using the recommended number of differences of 1 on Italy's GDP the Augmented Dickey-Fuller (ADF) test suggested marginal evidence against stationarity with a p-value of 0.0675, as well as no further differencing. Since this is not completely stationary the 4th difference is take, because a seasonal component was detected in the ploted data as well as the ACF and PACF. In the seasonal scenario, the ADF test provided strong evidence for stationarity with a p-value of 0.01. This suggests to use one difference in the modeling part, as well as including a SAR. (TODO is SAR right?)
...
analyze_diff_series(diff(transformed_usa$gdp), "USA", 'GDP')
...
plot_volatility_clusters(transformed_italy$stocks, 'Italy', 'Stocks', 200)
```
...
TODO description why which model and evaluation - first max lags - second accounting fro season in ACF and substracting -Other auto arma with differen IC
...
plot_residuals_arima <- function(model, country, variable){ 
  ...
  # ACF and PACF plots
  acf(model$residuals, main = paste("ACF of", variable, "in", country))
  pacf(model$residuals, main = paste("PACF of", variable, "in", country))
}


# Plot the best model TODO Yannik maybe in tow seperate parts?
plot_residuals_arima(model_bic_i_gdp, "Italy", "GDP")
...
TODO Discription of good forcast

### 2.3.1 CPI Italy

QUESTION Heteroskedasticity so should we use GARCH or use log or maybe box cox? look at residuals
...
```{r garch model italy cpi, echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
# Plot the forecast
#plot_forecast_arima(cpi_italy_train, cpi_italy_test, transformed_italy$cpi, "Italy", "CPI", )# GARCH OR COMBINATION?) TODO
