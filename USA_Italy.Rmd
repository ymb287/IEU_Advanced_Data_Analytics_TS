...
After using the recommended number of differences (1) on USA GDP the Augmented Dickey-Fuller (ADF) test suggested evidence against stationarity with a p- value of 0.12, as well as no further differencing. Since this is not completely stationary it was differenced once more and then the ADF test provided strong evidence for stationarity. This suggests using one difference in the modeling part, and inlcuding a drift.
...
For USA cpi data the ADF test has a p-value of 0. 0.60 suggesting non-stationarity. Also taking one difference is recommended. The ACF shows a slow decline and since the PACF has a steep drop at lag 1, the model will probably be an AR(1). This will be tested in the modeling part.
...
For USA stock data the Augmented Dickey-Fuller test has a p-value of 0.  0.41. The ACF shows a slow decline and  the PACF has a steep drop at lag 1 and a probable violation at lag 6.
...
## 2.2 Trend and Seasonality with holt??? TODO
...
plot_sqr_residuals_arima <- function(model, country, variable){ 
  
  # Set up the plot
  par(mfrow = c(1, 2), mar = c(3, 4, 4, 1))

  # ACF and PACF plots
  acf((model$residuals)^2, main = paste("ACF of", variable, "squared residuals in", country))
  pacf((model$residuals)^2, main = paste("PACF of", variable, "squared residuals in", country))
}


# Plot the best model TODO Yannik maybe in tow seperate parts?
plot_sqr_residuals_arima(model_bic_i_cpi, "Italy", "GDP")

```

TODO Discribe ACF gives ARCH(p) and PACF gives q in GARCH(p,q)
...
a1 <- ugarchfit(ugarchspec(variance.model = list(model = "sGARCH", garchOrder = c(1, 0)), distribution.model = "std"), data = cpi_italy_train)
a2 <- ugarchfit(ugarchspec(variance.model = list(model = "sGARCH", garchOrder = c(2, 0)), distribution.model = "std"), data = cpi_italy_train)
a3 <- ugarchfit(ugarchspec(variance.model = list(model = "sGARCH", garchOrder = c(3, 0)), distribution.model = "std"), data = cpi_italy_train)
a4 <- ugarchfit(ugarchspec(variance.model = list(model = "sGARCH", garchOrder = c(4, 0)), distribution.model = "std"), data = cpi_italy_train)
...
model_list_garch <- list(a1, a2, a3, a4, a5, a6, a7, a8, a9, g1, g2)
